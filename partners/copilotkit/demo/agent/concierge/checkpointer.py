...
async def init_checkpointer() -> None:
    ...
    # Idempotent: a repeat call (uvicorn --reload re-entry, a test, a future
    # warm-restart hook) must not overwrite an already-open pool — that would
    # orphan it with no close(). A prior *failed* init leaves both globals None,
    # so a retry after a transient error is still allowed.
    ...
    try:
        ...
        # Close the pool we already opened before abandoning it. create_pool_async
        # eagerly opens connections (min=1), so nulling _pool without closing it
        # would leak the Oracle session(s) for the process lifetime.
...
def resolve_checkpointer() -> BaseCheckpointSaver:
